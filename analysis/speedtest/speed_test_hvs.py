...
if __name__ == "__main__":
    ...
    select_fn = MinSim(
        "cross", student, teacher,
        dino_loss, fp16_scaler,
        4, 16, local_crops_number, 128
    )

    images = [torch.randn(bs, 3, 224, 224).cuda() for _ in range(4)]
    images += [torch.randn(bs, 3, 96, 96).cuda() for _ in range(16)]
    ...
    def test_fn(images):
        for _ in range(100):
            ...
            with autocast():
                teacher_output = teacher(inputs[:2])  # only the 2 global views pass through the teacher
