...
def finetune(
    ...
    linear_classifier = nn.Linear(embed_dim, num_classes)
    ...
    utils.restart_from_checkpoint(
        os.path.join(output_dir, "finetune_checkpoint.pth"),
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if dist.is_main_process():
            ...
            torch.save(save_dict, os.path.join(output_dir, "finetune_checkpoint.pth"))
...
def get_finetune_args_parser():
    p = argparse.ArgumentParser(description="finetune eval")
    p.add_argument("--arch", default="resnet50", type=str, help="backbone architecture")
    p.add_argument("--arch_kwargs", default=None, type=json.loads, help="backbone architecture kwargs")
    p.add_argument("--batch-size", default=4096, type=int, help="total batch size")
    p.add_argument("--epochs", default=30, type=int, help="number of total epochs to run")
    ...
    p.add_argument("--output-dir", default=".", type=str, help="path to save checkpoints")
    ...
    p.add_argument("--val-freq", default=1, type=int, help="validation frequency")
    return p
